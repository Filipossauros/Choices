...
export interface ModelSpec {
  ...
  decision: { bands: { label: string; minScore: number; referenceProfile?: Record<string, string> }[] };
  diagnostics: {
    leafCount: number;
    factorCount: number;
    gateCount: number;
  ...
  return {
    ...
    decision: {
      bands: [...model.decisionScale]
        ...
        .map((b) => ({ label: b.label, minScore: b.minScore, referenceProfile: b.referenceProfile })),
    },
    diagnostics: {
      leafCount,
      factorCount,
...
export function buildCapabilitiesManifest(): CapabilitiesManifest {
  return {
    ...
    capabilities: [
      ...
      'Derivação de escalas cardinais de valor por MACBETH com verificação de consistência (LP).',
      ...
      'Análise de sensibilidade dos pesos e deteção de mudanças de ordenação.',
      'Exportação de modelo, avaliação, especificação para IA (JSON) e relatório (PDF).',
