...
export default function Sensitivity() {
  ...
  const allValues = (scenario?.points ?? []).flatMap((pt) =>
    optionIds.map((id) => pt.optionValues[id] ?? 0),
  );
  const yMin = allValues.length ? Math.floor(Math.min(...allValues, model.conditionalThreshold) - 10) : -20;
  const yMax = allValues.length ? Math.ceil(Math.max(...allValues, 100) + 5) : 110;

  // Label for the current-weight vertical reference line
  ...
  return (
    <div className="max-w-5xl mx-auto py-6 px-4 space-y-6">
            ...
            <ResponsiveContainer width="100%" height={300}>
              <LineChart data={chartData} margin={{ top: 4, right: 16, bottom: 4, left: 0 }}>
                ...
                <ReferenceLine y={model.approvedThreshold} stroke="#16a34a" strokeDasharray="4 4"
                  label={{ value: 'Recomendado', position: 'insideTopRight', fontSize: 9, fill: '#16a34a' }} />
                <ReferenceLine y={model.conditionalThreshold} stroke="#d97706" strokeDasharray="4 4"
                  label={{ value: 'Com reservas', position: 'insideTopRight', fontSize: 9, fill: '#d97706' }} />
                ...
                {scenario.rankingChangePoints.map((w, i) => {
                  ...
                  return (
                    <li key={i} className="flex items-start gap-2">
                      <span className="font-mono text-amber-900 shrink-0">
                        {variedCrit?.label} = {(w * 100).toFixed(1)}%
                      </span>
                      {swaps.length > 0 && (
                        <span className="text-amber-700">
                          — {swaps.map((s) => optionLabels.get(s.id)).join(' ↔ ')} trocam de posição
                        </span>
                      )}
                    </li>
                  );
      ...
      <ScreenNav
        next="report"
        nextLabel="Relatório"
        hint="Gere e exporte o relatório de decisão."
      />
    </div>
  );
}
